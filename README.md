...
- Razor Views + Tailwind CSS — interface
...
│       ├── Data/                     # DbContext, repositórios e suas interfaces (EF Core + SQLite)
...
- `TabelaPreco` — parametrização de valores por vigência, com suas próprias validações de construção
...
A camada `Web` cuida da persistência (EF Core + SQLite, acesso assíncrono, repositórios expostos por interface) e da interface (Controllers e Views), orquestrando as classes de domínio através do serviço `RegistradorDeSaida`.
